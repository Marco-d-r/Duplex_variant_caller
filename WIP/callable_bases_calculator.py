...
Output CSV preserves every column from the input CSV and appends:
  ...
  (identical set of four columns repeated for the padded BED)
...
def compute_depth_stats(
    ...
    Pipeline
    --------
    samtools view (read filters) → samtools depth -a (per-position depth)

    Read filters applied — all three must pass before a read contributes
    any depth to any position:

      1. FLAG -F 3844  — excludes unmapped (4), non-primary (256), QC-failed
                         (512), duplicate (1024), supplementary (2048) reads.
      2. -q min_mapq   — mapping quality threshold (default 40).
      3. -e expression — AS − XS ≥ min_asxs (default 50).  Reads lacking an
                         XS tag (i.e. uniquely mapping reads) have XS treated
                         as 0 and therefore always pass.

    The BED passed as region_bed must already have blacklisted intervals
    removed (done once upstream); samtools depth -a only iterates over
    positions inside that BED, so blacklisted sites never appear.

    Counting philosophy
    -------------------
    Each position's depth (number of reads passing all filters that overlap
    that base) is accumulated rather than counted as ±1.  So:

      total_sequenced_bases    = Σ depth   over ALL positions in region_bed
      callable_sequenced_bases = Σ depth   over positions where depth ≥ min_depth

    If 100 filtered reads each 150 bp long fully cover a region, that region
    contributes 15,000 to callable_sequenced_bases — not 150.

    Returns
    -------
    dict with keys:
        callable_sequenced_bases  – mutation-rate denominator
        total_sequenced_bases     – total filtered reads × overlap bp
        mean_depth                – mean per-position depth over the BED
        total_positions           – number of genomic positions in region_bed
    """
    asxs_expr = f"(exists([XS]) && ([AS] - [XS] >= {min_asxs})) || (!exists([XS]) && ([AS] >= {min_asxs}))"
    #asxs_expr = f"[AS] >= {min_asxs}"

    # samtools view applies all read-level filters; the filtered BAM stream
    # is piped directly to samtools depth so no low-quality read can
    # contribute depth to any position.
    depth_cmd = (
