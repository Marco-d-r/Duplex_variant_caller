...
def _process_sample(args: Tuple) -> Dict[str, Any]:
    ...
    # Amount of bases to trim from both ends of the reads for the callable depth
    TRIM_BP = 4
    ...
    for chrom, reg_start, reg_end in wes_territory_intervals:
        ...
        try:
            for read in bam.fetch(chrom, reg_start, reg_end):
                ...
                # 1. TOTAL BASES LOGIC (No trimming)
                ...
                # 2. CALLABLE BASES LOGIC (With trimming)
                adj_start = read.reference_start + TRIM_BP
                adj_end   = ref_end - TRIM_BP

                # Clamp trimmed read span to this region
                s_filt = max(adj_start, reg_start) - reg_start
                e_filt = min(adj_end,   reg_end)   - reg_start
                
                # Only record to filtered depth if the trimmed read actually 
                # spans an area in this region AND passes your quality filters
                if s_filt < e_filt:
